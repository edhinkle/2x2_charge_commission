...
for f in ${JSON_DIR}/File_correctedConfig_${DATE1}_*_CDT_BATCH*.json; do
    echo "Processing $f"
    python August2025_Charge_Commission_make_pedestal_plots.py -cd $f -n ${DATASET1_NAME} -d ${DATE1} -idx ${NOM_DSET_IDX} -l ${YAML_MOD0} ${YAML_MOD1} ${YAML_MOD2} ${YAML_MOD3} -mo 0 1 2 3 -mm ${MAX_MEAN} -ms ${MAX_STD}
done
