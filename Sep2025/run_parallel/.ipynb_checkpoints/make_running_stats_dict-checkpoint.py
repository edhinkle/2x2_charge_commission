...
def make_channel_running_stats_dict_one_file(h5_file, dataset_name, date, max_entries=-1):
    ...
    unique_id_set_valid, packet_counts = np.unique(unique_ids_valid, return_counts=True)
    # identifying the channel_ids which are particularly noisy and remove them from array
    # noisy_ids = np.array([])
    # for uid, count in zip(unique_id_set_valid, packet_counts):
    #     if count > 10000:
    #         noisy_ids = np.append(noisy_ids, uid)
    #         print(uid, ' ', count)
    # unique_id_set_valid = np.setdiff1d(unique_id_set_valid, noisy_ids)
    ...
    for uid, row in tqdm.tqdm(valid_stats.iterrows()):
        stats = channel_running_stats_dict[uid]
        stats['sum'] += row['sum']
...
def main(h5_file=None, idx=None, dataset_name=None, date=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--h5_file', default=None, type=str,help='''data file being dictionaried.''')
    parser.add_argument('-i','--idx', default=None,type=int,help='''Index of file to be processed.''')
